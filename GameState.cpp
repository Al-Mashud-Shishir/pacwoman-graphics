...
{
	m_text.setFont (game->getFont ());
	m_text.setString ("Press Start (S) when you're ready...");
...
void PlayingState::loadNextLevel ()
{
	...
	for (auto ghostPosition : m_maze.getGhostPositions ())
	{
		...
		//ghost->setPosition (m_maze.mapCellToPixel (sf::Vector2i(0,0)));

		m_ghosts.push_back (ghost);
	}

	// Change speed according to the new level
	float speed = 100 + (speedLevel * 5);
...
void PlayingState::update (sf::Time delta)
{
	...
	if (offset.x <= 2 && offset.x >= -2 && offset.y <= 2 && offset.y >= -2)
	{
		...
		{
			m_score += 5;
			m_pacWoman->setSpeed (m_pacWoman->getSpeed () + 5);
		}
...
void LostState::update (sf::Time delta)
{
	...
	m_countDownText.setString ("Insert a coin to continue...Press I " + std::to_string (10 - static_cast<int>(m_countDown.asSeconds ())));
